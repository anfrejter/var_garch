...
Based on Breusch Godfrey test we cannot reject hypothesis that there is no autocorrelation among residuals for DF test with 0 augumentations. P-value for ADF test with zero augumentations is low - we can reject hupothesis that time series is non stationary. Residuals of ARIMA(0,0) is stationary.
...
Finally model seems to be correct in terms of autocorrelation of standarized squared residuals and arch effects. Autocorrelation based on ACF plot and Ljung-Box test are no longer significant. Additionally, investigating ACF plot of standarized residuals we can see that none of lags up. to 15th have statistically significant autocorrelation. ARCH of order 9 is quite a lot, we will try fitting ARCH models of order 1 to 9 without such detailed analysis.
...
Nyblom stavility test tells us that model parameters are jointly unstable, but looking at it individually, only omega is likely to change in time.
...
As leverage effect is present we will additionally try fitting EGARCH and APARCH (recommended in documentation) models which takes this issue into account. Since for all Jarque-Bera tests null hypothesis about normal distribution among standarized residuals was rejected, we will try working with distributions such as t-student, skewed t-student, generalized error, skewed generalized error, skewed normal.
...
If we consider Akaike ans Shibata scores, the second best model is
...
-   all the parameters are significant except for omega and mu (intercept of variance equation and mean equation). Leverage parameter, gamma is significant with positive estimate meaning that neagtive innovation has higher effect than positive innovations.
...
Based on ACF of standardized residuals none of lags untill 15 is autocorrelated, based on ACF on squared tandardized residuals variance model is correct - no significant autocorrelation until 21 lag, 5th lag is on the verge of the significance. News impact curve shows us that the impact of innovation on volatility is not symmetric - impact of negative innovation on volatility is higher than for positive impacts. Compating to EGARCH we can see that New Impact Cruve is more convex. QQ plot seems to be correct - t distribution matches data distribution.
