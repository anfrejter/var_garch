...
p1 = ggplot(fortify(SNEUCX)) +
  geom_line(aes(x=Index, y=CumRet), colour = 'black') +
...
The analysis reveals that the distribution of the created index does not follow a normal distribution. The log returns are more concentrated around the average value, resulting in a higher peak at the mean. This leptokurtosis suggests a higher likelihood of outliers compared to a normal distribution. It indicates heavy tails and a greater probability of extreme returns. This observation highlights the importance of accounting for potential risks and the variability in returns when evaluating investment performance.
...
The x-axis plots the theoretical quantiles. Those are the quantiles from the standard Normal distribution with mean 0 and standard deviation 1. Black points don't follow the red line. That means that heavy tails are present. It means that, compared to the normal distribution, there is much more data located at the extremes of the distribution and less data in the center of the distribution
...
Value of Kurtosis is equal to 11.75, which is grater than 3. That idicates that distribution of data set does not follow normal distribution.
